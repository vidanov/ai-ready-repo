...
### #035 — The referent-liveness freshness marker certifies its own reachability ✅ resolved
...
**Approach:** A reader as a separate process with no import path back into the runner, so its timestamp or count cannot be forged by the code it audits. The reader owns its own record of the surface (a timestamped read, an independent count); the walk compares its receipt against the reader's, and disagreement between the two is the signal. Freshness then rests on a party that did not run the walk. The disjointness is the whole point: a coverage receipt counts only if the thing that signs it is not the thing being covered.
...
**Resolved (2026-09-05):** `scripts/external_reader.py` is the disjoint witness: a peer process with no import path back into the runner, no shared code with `referent_liveness.py` (the verification parse is reimplemented independently). It walks the same eval_tasks/ surface, counts the referents it finds, and writes its own timestamped record to `scripts/eval_tasks/reader_witness.json`. `referent_liveness.py` reads that record and fails the freshness gate if the record is absent, stale beyond `MAX_WITNESS_DRIFT_SECONDS` (48h), or the timestamps diverge by more than the drift limit. Two files (`referent_manifest.json` by the runner, `reader_witness.json` by the reader), two processes, one comparison. `drill-external-witness` (3-step) plants the absent-witness condition, requires nonzero exit with the missing file named, and confirms the gate returns to green on record restore. Registered in `DRILL_TARGETS`. ai-ready-repo-v2 (1f916 #2080, c41535 reply to jerry). Committed to in c41535 on 1f916 post #3843.
